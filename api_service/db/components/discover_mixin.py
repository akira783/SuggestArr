"""Per-user storage for the Discover feature: card votes and the AI taste profile.
...
class DiscoverMixin:
    DISCOVER_VOTES = ('like', 'dislike', 'seen_liked', 'seen_disliked')
    DISCOVER_POSITIVE_VOTES = ('like', 'seen_liked')
    DISCOVER_PICK_TYPES = ('safe', 'explore', 'calibration')
    DISCOVER_MEDIA_TYPES = ('movie', 'tv')

    def _discover_placeholder(self):
        return '%s' if self.db_type in ('mysql', 'mariadb', 'postgres') else '?'

    @staticmethod
    def _discover_timestamp(value):
        return None if value is None else str(value)

    def _validate_discover_media_type(self, media_type):
        if media_type not in self.DISCOVER_MEDIA_TYPES:
            raise ValueError("media_type must be 'movie' or 'tv'")

    def set_discover_vote(self, user_id, tmdb_id, media_type, vote, title=None, year=None,
                          genres=None, rationale=None, pick_type=None):
        """Insert or update one user's vote on a Discover card.
        ...
        :param vote: One of ``DISCOVER_VOTES``.
        ...
        :param pick_type: One of ``DISCOVER_PICK_TYPES`` or None.
        ...
        """
        if vote not in self.DISCOVER_VOTES:
            raise ValueError(f"vote must be one of {', '.join(self.DISCOVER_VOTES)}")
        self._validate_discover_media_type(media_type)
        if pick_type is not None and pick_type not in self.DISCOVER_PICK_TYPES:
            raise ValueError(f"pick_type must be one of {', '.join(self.DISCOVER_PICK_TYPES)}")

        ph = self._discover_placeholder()
        ...
        if self.db_type in ('mysql', 'mariadb'):
            query = f"""
                INSERT INTO discover_votes {columns} {values}
            ...
            query = f"""
                ...
                ON CONFLICT(user_id, tmdb_id, media_type) DO UPDATE SET
                    vote=excluded.vote,
                    title=COALESCE(excluded.title, discover_votes.title),
                    year=COALESCE(excluded.year, discover_votes.year),
                    genres=COALESCE(excluded.genres, discover_votes.genres),
                    rationale=COALESCE(excluded.rationale, discover_votes.rationale),
                    pick_type=COALESCE(excluded.pick_type, discover_votes.pick_type),
    ...
    def mark_discover_requested(self, user_id, tmdb_id, media_type):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"UPDATE discover_votes SET requested=1, updated_at=CURRENT_TIMESTAMP "
    ...
    def get_discover_votes(self, user_id, limit=None, media_type=None):
        ...
        query = (
            ...
            "requested, created_at, updated_at FROM discover_votes WHERE user_id=" + ph
        )
        params = [int(user_id)]
        ...
        for row in rows:
            ...
            votes.append({
                ...
                'created_at': self._discover_timestamp(row[9]),
                'updated_at': self._discover_timestamp(row[10]),
            })
        return votes

    def get_discover_voted_ids(self, user_id, media_type=None):
        """Return the TMDb ids a user already voted on, to exclude them from new cards.

        :return: Set of ``(tmdb_id, media_type)`` tuples.
        """
        ph = self._discover_placeholder()
        query = f"SELECT tmdb_id, media_type FROM discover_votes WHERE user_id={ph}"
        params = [int(user_id)]
        if media_type is not None:
            self._validate_discover_media_type(media_type)
            query += f" AND media_type={ph}"
    ...
    def count_discover_votes(self, user_id):
        """Return how many cards a user has voted on (drives the Calibration mode)."""
        ph = self._discover_placeholder()
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(f"SELECT COUNT(*) FROM discover_votes WHERE user_id={ph}", (int(user_id),))
    ...
    def get_discover_stats(self, user_id):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(
                f"SELECT vote, pick_type, requested FROM discover_votes WHERE user_id={ph}",
                (int(user_id),),
            )
            rows = cursor.fetchall()
        total = len(rows)
        likes = sum(1 for vote, _, _ in rows if vote in self.DISCOVER_POSITIVE_VOTES)
        ...
        for vote, pick_type, _ in rows:
            ...
            bucket['total'] += 1
            if vote in self.DISCOVER_POSITIVE_VOTES:
                bucket['likes'] += 1
        return {
            'total': total,
            'likes': likes,
    ...
    def clear_discover_votes(self, user_id):
        ...
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(f"DELETE FROM discover_votes WHERE user_id={ph}", (int(user_id),))
            deleted = cursor.rowcount
            cursor.execute(
                f"UPDATE discover_taste_profile SET votes_since_update=0 WHERE user_id={ph}",
                (int(user_id),),
            )
            conn.commit()
        return deleted
    ...
    def get_taste_profile(self, user_id):
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                f"FROM discover_taste_profile WHERE user_id={ph}",
                (int(user_id),),
            )
            row = cursor.fetchone()
        ...
        return {
            ...
            'updated_at': self._discover_timestamp(row[3]),
        }
    ...
    def save_taste_profile(self, user_id, profile_text, user_edited=False):
        ...
        if self.db_type in ('mysql', 'mariadb'):
            query = f"""
                INSERT INTO discover_taste_profile (user_id, profile_text, votes_since_update, user_edited)
                VALUES ({ph}, {ph}, 0, {ph})
    ...
    def increment_taste_profile_votes(self, user_id):
        ...
        if self.db_type in ('mysql', 'mariadb'):
            upsert = f"""
                INSERT INTO discover_taste_profile (user_id, profile_text, votes_since_update)
                VALUES ({ph}, '', 1)
            ...
            upsert = f"""
                ...
                ON CONFLICT(user_id) DO UPDATE SET
                    votes_since_update=discover_taste_profile.votes_since_update + 1
            """
        with self.get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute(upsert, (int(user_id),))
            cursor.execute(
                f"SELECT votes_since_update FROM discover_taste_profile WHERE user_id={ph}",
                (int(user_id),),
            )
